"""Block 2 (3/5) -- Accessible.

Detail diagram behind the "Accessible" badge. Mechanism per PRIMER.md A4
(proposal, not yet checked against the deRSE26 paper's FAIR table).

Produces:
  img/block-2-fair4rs-chain/fair-accessible-detail.svg / .png (transparent)

Runnable standalone: `python py/step_fair_accessible.py`
"""
...
def _build_svg() -> tuple[str, float, float]:
    ...
    p.append(label_box(
        ...
        [
            ("Wikibase API & SPARQL endpoint", 700, 24),
            ("No login required for reads", 700, 24),
            ("= open access today; a chublets Wikibase login only gates writes", 400, 17),
        ],
...
def run(strict: bool = False) -> list[str]:
    ...
    svg_path = BLOCK2_DIR / "fair-accessible-detail.svg"
