"""Block 2 (2/5) -- Findable.

Detail diagram behind the "Findable" badge. Mechanism per PRIMER.md A4
(proposal, not yet checked against the deRSE26 paper's FAIR table).

Produces:
  img/block-2-fair4rs-chain/fair-findable-detail.svg / .png (transparent)

Runnable standalone: `python py/step_fair_findable.py`
"""
...
def _build_svg() -> tuple[str, float, float]:
    ...
    p.append(label_box(
        ...
        [
            ("Persistent Wikidata Q-IDs", 700, 24),
            ("Indexed by nfdi.software & find.software", 700, 24),
            ("= discoverable by Q-ID, by SPARQL, and by cross-domain search", 400, 17),
        ],
...
def run(strict: bool = False) -> list[str]:
    ...
    svg_path = BLOCK2_DIR / "fair-findable-detail.svg"
